...
def sign_detector(input_img, if_show):
    ...
    ret, contours, hierarchy = cv2.findContours(img_blue,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)#contours为轮廓集，可以计算轮廓的长度、面积等
...
if __name__ == "__main__":
    #加载图像img
    img = cv2.imread('/home/ubuntu/imges_for_test/signs/IMG_3134 Medium.jpeg',cv2.IMREAD_COLOR)
    # sign_read(img)
    sign_detector(img,0)
